...
class PyFingerprint(object):
    ...
    Arguments:
        packetType (int): The packet type (e.g., FINGERPRINT_COMMANDPACKET, FINGERPRINT_DATAPACKET, etc.)
        packetPayload (tuple): The payload
        """

    # Convert Start Code, Address, and Packet Type to bytes and write
    ...
    # Calculate packet length and write it as 2 bytes
        packetLength = len(packetPayload) + 2
        packetLengthHigh = self.__rightShift(packetLength, 8)
        packetLengthLow = self.__rightShift(packetLength, 0)
        self.__serial.write(self.__byteToString(packetLengthHigh))
        self.__serial.write(self.__byteToString(packetLengthLow))

    # Calculate the checksum including packetType, packet length, and payload bytes
        packetChecksum = (packetType + packetLengthHigh + packetLengthLow + sum(packetPayload)) & 0xFFFF

    # Debugging information for checksum
        print(f"Calculated Checksum: {hex(packetChecksum)}")

    # Write each payload byte
        for byte in packetPayload:
            self.__serial.write(self.__byteToString(byte))

    # Write checksum as 2 bytes (high and low bytes)
    ...
    def __readPacket(self, timeout=10.0):
        """
    ...
    Returns:
        ...
        1: list of integers (n bytes) The packet payload.

    Raises:
        Exception: if checksum is wrong or other errors occur.
        """
        ...
        while True:
            if time.time() - start_time > timeout:
                raise Exception("The received packet is incomplete!")
            ...
            if i >= 12:
                ...
                packetPayloadLength = self.__leftShift(receivedPacketData[7], 8) | receivedPacketData[8]
                
                if i < packetPayloadLength + 9:
                    continue

                packetType = receivedPacketData[6]
                packetChecksum = (packetType + receivedPacketData[7] + receivedPacketData[8]) & 0xFFFF
                packetPayload = []

                for j in range(9, 9 + packetPayloadLength - 2):
                    packetPayload.append(receivedPacketData[j])
                    packetChecksum = (packetChecksum + receivedPacketData[j]) & 0xFFFF
                    
                if len(receivedPacketData) < (9 + packetPayloadLength):
                    raise Exception("The received packet does not contain enough data for checksum validation!")

                receivedChecksum = (self.__leftShift(receivedPacketData[9 + packetPayloadLength - 2], 8) |
                                receivedPacketData[9 + packetPayloadLength - 1])
                
                print(f"Calculated Checksum: {hex(packetChecksum)}, Received Checksum: {hex(receivedChecksum)}")
                print("Final two bytes for checksum:", 
                      hex(receivedPacketData[9 + packetPayloadLength - 2]), 
                      hex(receivedPacketData[9 + packetPayloadLength - 1]))
                
                if receivedChecksum != packetChecksum:
                    raise Exception("The received packet is corrupted (checksum mismatch)!")
    ...
    # Send the verification command to the sensor
        self.__writePacket(FINGERPRINT_COMMANDPACKET, packetPayload)
        time.sleep(0.2)  # Allow time for response

        try:
        # Try to read the packet from the sensor with increased timeout
            receivedPacket = self.__readPacket(timeout=5.0)  

        # Unpack the received packet details
            receivedPacketType = receivedPacket[0]
            receivedPacketPayload = receivedPacket[1]

        # Debug: Display the received packet type and payload
            print("Received packet type:", hex(receivedPacketType))
            print("Received packet payload:", [hex(byte) for byte in receivedPacketPayload])
    ...
    def setPacketSize(self, packetSize):
        """Sets the packet size on the sensor to 32, 64, or 128 bytes."""
        packetPayload = (
            FINGERPRINT_SETSYSTEMPARAMETER,
        ...
        if receivedPacketPayload[0] == FINGERPRINT_OK:
            print(f"Packet size set to {packetSize} bytes.")
        else:
            print("Failed to set packet size.")
    ...
    # Check for valid buffer number
        if charBufferNumber not in [FINGERPRINT_CHARBUFFER1, FINGERPRINT_CHARBUFFER2]:
            raise ValueError('Invalid char buffer number!')

        packetPayload = (FINGERPRINT_DOWNLOADCHARACTERISTICS, charBufferNumber)
        self.__writePacket(FINGERPRINT_COMMANDPACKET, packetPayload)
        print("Sending initial packet for downloading characteristics...")

        try:
            receivedPacket = self.__readPacket(timeout=5.0)
            receivedPacketType = receivedPacket[0]
            receivedPacketPayload = receivedPacket[1]
            print(f"Received packet type: {hex(receivedPacketType)}, payload: {receivedPacketPayload}")
        except Exception as e:
            raise Exception("Failed to initialize characteristics download.")

        if receivedPacketType == FINGERPRINT_ACKPACKET:
            if receivedPacketPayload[0] != FINGERPRINT_OK:
                self._handle_error(receivedPacketPayload[0])
        elif receivedPacketType == FINGERPRINT_DATAPACKET:
            completePayload = []
            completePayload.extend(receivedPacketPayload)
        else:
        ...
        while not packet_received:
            ...
            #if time.time() - start_time > timeout:
                #raise Exception("Timeout: Did not receive all packets in time.")
        ...
        # Append data if it's a data packet, break if end data packet
                if receivedPacketType == FINGERPRINT_DATAPACKET:
                    completePayload.extend(receivedPacketPayload)
                elif receivedPacketType == FINGERPRINT_ENDDATAPACKET:
                    completePayload.extend(receivedPacketPayload)
                    print("End data packet received, download complete.")
                    packet_received = True
                else:
                    raise Exception("Unexpected packet type during download.")
            
            except Exception as e:
                print(f"Error during packet reading: {e}")
                retry_count += 1
                if retry_count > 5:
                    raise Exception("Failed to receive complete packet after multiple attempts.")
                print(f"Retrying... ({retry_count}/3)")
                
            #print("Complete payload length:", len(completePayload))
        print("Final completePayload length:", len(completePayload))
        if len(completePayload) != expected_size:
            print(f"Warning: Received payload size ({len(completePayload)}) does not match expected size ({expected_size}).")
        else:
            print("Payload size matches expected size.")
